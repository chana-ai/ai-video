...
export default function CreateMaterial() {
  ...
  return (
    <div className="max-w-screen-xl mx-auto my-5">
      ...
      <div>
          <div className="mt-5"><label>名字:
            <input type="text" value={name} onChange={handleNameChange} />
          </label></div>
          <div className="mt-5"><label>标签(用,区分):
            <input type="text" value={tags} onChange={handleTagsChange} />
          </label>
          </div>
        
        <div className="mt-7">
          <input type="file" multiple accept="image/*,video/*" className="bg-blue-800 text-white mr-5" onChange={handleFileUploadChange} ref={fileUpload} />
        ...
        <div className="mt-5">
          {fileData && fileData.length > 0 && (
            fileData.map((item:any, index:any)=>{
              return(
                <div key={index}>{index+1} - {item.name} -  {fileUploadStatus && fileUploadStatus.length &&  ( "File Uploaded" )} </div>
              )
            })
          )}
        ...
        <div>
            <><button className="bg-blue-800 text-white rounded-md py-1 px-4 hover:bg-green-700" onClick={handleSubmit}>提交</button> </>
            <><button className="bg-blue-800 text-white rounded-md py-1 px-4 hover:bg-green-700" onClick={handleCancel}>取消</button> </>
          </div>
        </div>
