...
import { Loader2, RotateCcw, Star, Clock, ChevronLeft, ChevronRight } from 'lucide-react'
...
export const ImageConfigTab: React.FC<ImageConfigTabProps> = ({
    ...
    const [zoomedImageId, setZoomedImageId] = React.useState<number | null>(null)
    ...
    const handleGenerateClick = () => {
        ...
        } else if (generateSideBack) {
            alert('Choosing one reference image')
        }
    ...
    const toggleImageSelection = (id: number) => {
        // 切换选中状态：如果当前已选中则取消选中，否则选中当前图片
        if (selectedImageIds.has(id)) {
            // 取消选中（空集合）
            onSetSelectedImageIds(new Set())
        } else {
            // 选中当前图片，清除其他选中
            onSetSelectedImageIds(new Set([id]))
        }
    }
    ...
    // Render grid when no image is zoomed
            ...
            {currentBatch.images.map((img) => {
                ...
                return (
                    ...
                    >
                        ...
                        {/* Selection indicator circle - independent from zoom */}
                        <div
                            className={cn(
                                "absolute top-2 right-2 rounded-full transition-all duration-200 cursor-pointer",
                                "w-5 h-5 flex items-center justify-center",
                                isSelected
                                    ? "bg-green-500 shadow-sm"  // Filled green when selected
                                    : "bg-white/50 border-2 border-gray-300 hover:border-green-400"  // Transparent with border when not selected
                            )}
                            onClick={(e) => {
                                e.stopPropagation();
    ...
    // Render zoomed view overlay
    const renderZoomView = () => {
        const zoomedImage = currentBatch?.images.find(img => img.id === zoomedImageId);
